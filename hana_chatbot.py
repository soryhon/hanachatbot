...
def process_uploaded_file(uploaded_file):
    if uploaded_file is not None:
        ...
        print(f"총 {len(all_splits)}개의 passage")
...
def generate_response(query_text, vectorstore, callback):
    ...
    return response.content if hasattr(response, 'content') else "".join([r.content for r in response])
...
def generate_summarize(raw_text, callback):
    ...
    summary_prompt = [
        SystemMessage(content="주어진 텍스트를 간단히 요약해줘."),
        HumanMessage(content=f"텍스트: {raw_text}"),
    ]
