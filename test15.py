...
def get_github_files(repo, github_token, folder_name=None, branch="main"):
    ...
    if response.status_code == 200:
        ...
        st.warning(f"GitHub 파일 목록을 가져오지 못했습니다: {response.status_code}")
        return []  # 빈 리스트 반환
...
with st.expander("참고 탬플릿 미리보기", expanded=True):
    ...
    with col5_2:
        if st.button("미리보기"):
            if file_path:
                st.write(f"미리보기 경로: {file_path}")  # 경로 출력
            else:
    ...
    if not template_files:  # template_files가 빈 경우 기본값 설정
        st.warning("템플릿 파일이 없습니다. 새 파일을 업로드하세요.")
        template_files = ["템플릿 파일 없음"]
