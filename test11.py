...
GITHUB_TOKEN = st.secrets["import streamlit as st
import pandas as pd
import os
import requests
from io import BytesIO
...
GITHUB_REPO = "https://github.com/soryhon/hanachatbot/"  # 수정: GitHub 저장소 경로
GITHUB_BRANCH = "main"  # 푸시할 브랜치 이름
GITHUB_TOKEN = st.secrets["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"]  # GitHub Personal Access Token을 Streamlit Secrets로 저장
...
with col1:
    ...
    if uploaded_files:
        for uploaded_file in uploaded_files:
            ...
            # GitHub에 파일 업로드
            upload_file_to_github(file_path, uploaded_file.name, GITHUB_TOKEN, GITHUB_REPO, GITHUB_BRANCH)
...
with col3:
    ...
    if uploaded_save_file is not None:
        ...
        st.success("데이터가 불러와졌습니다.")"]  # GitHub Personal Access Token을 Streamlit Secrets로 저장
