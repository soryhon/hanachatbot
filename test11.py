...
# GitHub 저장소 존재 여부 확인 함수
def check_github_repo(repo, github_token):
    ...
    if response.status_code == 200:
        st.success("GitHub 저장소 경로가 유효합니다.")
    else:
        st.error(f"GitHub 저장소를 찾을 수 없습니다: {response.status_code} - {response.json()}")
...
with col2:
    ...
    if st.session_state['github_repo'] is None:
        ...
        if st.button("저장소 정보 저장"):
            if github_repo:
                # 저장소 경로 유효성 검사
                check_github_repo(github_repo, st.session_state['api_key'])
