    ...
    with st.expander("📝 보고서 선택", expanded=st.session_state['check_report_03']):
        ...
        with tab1:
            ...
            with col1:
                st.write("")
                st.markdown(
                    "<p style='font-size:14px; font-weight:bold; color:#000000f;text-align:center;background-color:#E7EAF1;margin-top:10px;border-radius:5px;'>등록된<br/>보고서명 선택 </p>",
                    unsafe_allow_html=True
                )
            with col2:
                # 폴더 존재 확인 및 생성
        ...
        with col1:   
            if st.button("💾 결과 내용 저장", key="save_result", use_container_width=True):
                ...
                if "response_03" in st.session_state:                
                    ...
                    github_folder = f"audioReportFiles/{folder_name}/{report_date_str}"
