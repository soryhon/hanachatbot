...
if github_info_loaded:
    with st.expander("✏️ 만족도 평가하기", expanded=True):
        ...
        with col1:  # score 1점 별
            st.image(star_images[0], width=120)
        with col2:  # score 2점 별
            st.image(star_images[1], width=120)
        with col3:  # score 3점 별
            st.image(star_images[2], width=120)
        with col4:  # score 4점 별
            st.image(star_images[3], width=120)
        with col5:  # score 5점 별
            st.image(star_images[4], width=120)
